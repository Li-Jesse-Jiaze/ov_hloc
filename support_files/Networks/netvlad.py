...
netvlad_path = Path(__file__).parent / 'weights'
...
class NetVLAD(nn.Module):
    ...
    def __init__(self, model_name='VGG16-NetVLAD-Pitts30K', checkpoint_dir=netvlad_path, whiten=True):
        super().__init__()

        assert model_name in self.dir_models.keys()

        # Download the checkpoint.
        checkpoint = checkpoint_dir / str(model_name + '.mat')
        ...
        self.mean = torch.from_numpy(mat['net'].meta.normalization.averageImage[0, 0]).float()
    ...
    def forward(self, data):
        ...
        image = image - self.mean.view(1, -1, 1, 1).to(image.device)
        # image = image / image.new_tensor(self.std).view(1, -1, 1, 1)
        image = image / self.std.view(1, -1, 1, 1).to(image.device)
