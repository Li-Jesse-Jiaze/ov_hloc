...
class SuperPoint(nn.Module):
    ...
    def forward(self, data):
        ...
        scores = [s[k.t()[0], k.t()[1]] for s, k in zip(scores, keypoints)]

        # Discard keypoints near the image borders
        noborders = [remove_borders(k, s, self.remove_borders, h*8, w*8) for k, s in zip(keypoints, scores)]
        keypoints = [n[0] for n in noborders]
        scores = [n[1] for n in noborders]

        # Keep the k keypoints with highest score
        if self.max_keypoints >= 0:
            top_k = [top_k_keypoints(k, s, self.max_keypoints) for k, s in zip(keypoints, scores)]
            keypoints = [n[0] for n in top_k]
            scores = [n[1] for n in top_k]
