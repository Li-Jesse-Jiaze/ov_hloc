...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    db.query(keyframe->global_descriptors, ret, 3, max_frame_id_allowed);
    //printf("query time: %f", t_query.toc());
    //cout << "Searching for Image " << frame_index << ". " << ret << endl;

    TicToc t_add;
    db.add(keyframe->global_descriptors);
    ...
    if (ret.size() >= 1 && ret[0].Score > LOOP_THRESHOLD)
        for (unsigned int i = 1; i < ret.size(); i++)
        {
            ...
            if (ret[i].Score > RELOC_THRESHOLD)
            {          
                find_loop = true;
                int tmp_index = ret[i].Id;
                if (DEBUG_IMAGE && 0)
...
*/
    if (find_loop && frame_index > 50)
    {
        if (LOAD_PREVIOUS_POSE_GRAPH){
            ...
            for (unsigned int i = 0; i < ret.size(); i++)
            {
                if (getKeyFrame(ret[i].Id)->sequence < keyframe->sequence)
                    return ret[0].Id;
                if (best_index == -1 || (ret[i].Id < best_index))
                    best_index = ret[i].Id;
                if (ret[i].Score <= RELOC_THRESHOLD) 
                    break;
            }
            ...
            for (unsigned int i = 0; i < ret.size(); i++)
            {
                if (min_index == -1 || (ret[i].Id < min_index && ret[i].Score > RELOC_THRESHOLD))
                    min_index = ret[i].Id;
            }
            return min_index;
        }
...
void PoseGraph::savePoseGraph()
{
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        std::string image_path, local_desc_path, global_desc_path, keypoints_path;
        if (DEBUG_IMAGE)
        {
        ...
        fprintf (pFile, " %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %f %f %f %f %f %f %f %f %d %d %d\n",(*it)->index, (*it)->time_stamp, 
                                    ...
                                    (int)(*it)->keypoints.size(), (*it)->height, (*it)->width);

        // write keypoints, brief_descriptors   vector<cv::KeyPoint> keypoints vector<BRIEF::bitset> brief_descriptors;
        local_desc_path = POSE_GRAPH_SAVE_PATH + to_string((*it)->index) + "_local_desc.dat";
        SaveMatBinary(local_desc_path, (*it)->local_descriptors);
        global_desc_path = POSE_GRAPH_SAVE_PATH + to_string((*it)->index) + "_global_desc.dat";
        SaveMatBinary(global_desc_path, (*it)->global_descriptors);
...
void PoseGraph::loadPoseGraph()
{
    ...
    int cnt = 0;
    while (fscanf(pFile,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %d %d %d", &index, &time_stamp, 
                                    ...
                                    &keypoints_num, &height, &width) != EOF) 
    {
        /*
        ...
        */
        ...
        string local_desc_path = POSE_GRAPH_SAVE_PATH + to_string(index) + "_local_desc.dat";
        string global_desc_path = POSE_GRAPH_SAVE_PATH + to_string(index) + "_global_desc.dat";
        ...
        for (int i = 0; i < keypoints_num; i++)
        {
            ...
            double p_x, p_y, p_x_norm, p_y_norm, score;
            if(!fscanf(keypoints_file,"%lf %lf %lf %lf %lf", &p_x, &p_y, &p_x_norm, &p_y_norm, &score))
        ...
        fclose(keypoints_file);
        LoadMatBinary(global_desc_path, global_descriptors);
        LoadMatBinary(local_desc_path, local_descriptors);

        KeyFrame* keyframe = new KeyFrame(time_stamp, index, VIO_T, VIO_R, PG_T, PG_R, image, loop_index, loop_info, keypoints, keypoints_norm, scores, local_descriptors, global_descriptors, height, width);
