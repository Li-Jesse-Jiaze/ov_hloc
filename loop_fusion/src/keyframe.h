#ifndef TEST_KEYFRAME_H
#define TEST_KEYFRAME_H
...
public:
    ...
    KeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, Vector3d &_T_w_i, Matrix3d &_R_w_i,
             ...
             std::vector<float> &_scores, cv::Mat &_local_descriptors, cv::Mat &_global_descriptors, int height, int width);
    bool findConnection(KeyFrame* old_kf);
    void computeWindow();
    void computeNew();
    void SuperGlueMatcher(
            std::vector<cv::Point2f> &matched_2d_old,
            std::vector<cv::Point2f> &matched_2d_old_norm,
            std::vector<uchar> &status,
            cv::Mat &local_descriptors_old,
            std::vector<float> &scores_old,
